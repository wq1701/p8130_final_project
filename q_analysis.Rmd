...
# coefficients change from 0.38530 to 0.34948, 
# not a confounder
```
...
therefore, we conclude that variables `dept`, `prate`, `clin`, `cert` and `exper` should be included in the model as confounders, they should be included in the model to adjust for the main predictor `gender`. `rank` is left. 

Now the model we have is: 

```{r}
model_1 = lm(log_mean_sal~gender + dept + prate + clin + cert + exper, data = law_df)
